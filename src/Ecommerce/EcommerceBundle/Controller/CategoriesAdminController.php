...
class CategoriesAdminController extends Controller
{
    ...
    public function createAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            $file = $entity->getImage()->getPath();

            $fileName = md5(uniqid()).'.'.$file->guessExtension();

            $file->move($this->container->getParameter('image_upload_directory'),$fileName);

            $entity->getImage()->setPath($fileName);
    ...
    private function createCreateForm(Categories $entity)
    {
        ...
        $form->add('submit', 'submit', array('label' => 'Creer','attr' => array('class' => 'button-success','style' => 'margin-top:10px;')));
    ...
    */
    private function createEditForm(Categories $entity)
    {
        ...
        $form->add('submit', 'submit', array('label' => 'Modifier','attr' => array('class' => 'button-success','style' => 'margin-top:10px;')));
    ...
    public function updateAction(Request $request, $id)
    {
        ...
        $oldimg = $this->container->getParameter('image_upload_directory').'/'.$entity->getImage()->getPath();
        ...
        if ($editForm->isValid()) {
            ...
            $file->move(
                $this->container->getParameter('image_upload_directory'),$fileName
            );
    ...
    public function deleteAction(Request $request, $id)
    {
        ...
        if ($form->isValid()) {
            ...
            if (file_exists($this->container->getParameter('image_upload_directory').'/'.$entity->getImage()->getPath())) 
            {
                unlink($this->container->getParameter('image_upload_directory').'/'.$entity->getImage()->getPath());
            }
    ...
    private function createDeleteForm($id)
    {
        return $this->createFormBuilder()
            ...
            ->setMethod('DELETE')
            ->add('submit', 'submit', array('label' => 'Supprimer','attr' => array('class' => 'button-danger','style' => 'float:right;')))
            ->getForm()
        ;
    }
}
